...
SELECT * FROM USER_TAB_COLUMNS WHERE TABLE_NAME = 'DEPT_COPY2';


--컬럼의 크기를 줄이는 경우
-- 변경하려는 크기를 초과하는 값이 없을때만 수정할 수 있다.
...
--지우려는 테이블에 최소 한개이상의 컬럼이 남아있어야한다.
ALTER TABLE DEPT_COPY2
DROP COLUMN DEPT_ID;
...
--컬럼 삭제시 참조하고 있는 철럼이 있다면 삭제할 수 없다.
ALTER TABLE TB1
DROP COLUMN PK;
...
SELECT * FROM USER_CONS_COLUMNS WHERE TABLE_NAME = 'DEPT_COPY3';
...
--테이블 삭제
DROP TABLE USER_GRADE2; --ORA-02449: unique/primary keys in table referenced by foreign keys
