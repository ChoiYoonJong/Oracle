...
-- 테이블에 일부컬럼에대해 INSERT
 ...
 INSERT INTO EMPLOYEE
    ...
    VALUES
    (
        900,'장채현','901123-1080503','jang@kh,co.kr','0105951234',
...
--INSERT 할때 VALUES 대시 서브쿼리 이용할 수 있다.
CREATE TABLE EMP_01(
    EMP_ID NUMBER,
    NAME VARCHAR2(30),
    DEPT_TITEL VARCHAR2(20)
);


SELECT * FROM EMP_01;

INSERT INTO TABLE EMP_01
(   
...
UPDATE EMP_SALRTY ES
SET ES.SALARY = (SELECT A.SALARY
                FROM EMP_SALRTY A
                WHERE A.EMP_NAME ='유재식'),
    ES.BONUS = (SELECT A.BONUS
                FROM EMP_SALRTY A
                WHERE A.EMP_NAME ='유재식')
...
UPDATE EMPLOYEE
SET EMP_NAME = NULL -- ORA-01407: cannot update ("EMPLOYEE"."EMPLOYEE"."EMP_NAME") to NULL
WHERE EMP_ID = 200;
...
DISABLE CONSTRAINT SYS_C007207 CASCADE;
...
SELECT * FROM EMP_SALRTY;
COMMIT;

DELETE FROM EMP_SALRTY;
ROLLBACK;

TRUNCATE TABLE EMP_SALRTY;
...
--테이터 삽입
INSERT INTO M_TEST01 VALUES('user11','김태연');
INSERT INTO M_TEST01 VALUES('user22','임찬희');
INSERT INTO M_TEST01 VALUES('user33','노한서');
...
--테이터 삽입
INSERT INTO M_TEST02 VALUES('user12','김연진');
INSERT INTO M_TEST02 VALUES('user22','조현');
INSERT INTO M_TEST02 VALUES('user32','한채영');
...
--MERGE 하기 (M_TEST01에 M_TEST02를 병합)

MERGE INTO M_TEST01 M1 USING M_TEST02 M2 ON(M1.ID = M2.ID)
WHEN MATCHED THEN -- 위에 ON 이 참이면 UPDATE 실행
UPDATE
SET M1.NAME = M2.NAME
WHEN NOT MATCHED THEN -- 위에 ON 이 참이면 INSERT 실행
INSERT 
    (M1.ID,M1.NAME)
VALUES
    (M2.ID,M2.NAME);