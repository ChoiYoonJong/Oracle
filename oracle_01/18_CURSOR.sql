...
is
    ...
    cursor c1 -- 커서 선언
    is
    select * from department; -- 조회한 결과를 c1에 저장
begin
    open c1; -- 커서열기
    loop
        fetch c1 --커서에 있는것을 한행씩 인출
        into v_dept.dept_id, -- 각변수에 담기
            v_dept.dept_title,
            v_dept.location_id;
        exit when c1%notfound; --커서가 더이상 다음행이 존재하지 않을때 빠져나감
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('성공한 객수 :' || V_COUNT_ROW);
END;
/
