...
--이때 나이가 적은 사람에서 많은 사람 순서로 화면에 출력되도록 만드시오. (단, 교수 중 2000 년 이후 출생자는 없으며 출력 헤더는 "교수이름", "나이"로 한다. 나이는 ‘만’으로 
--계산한다.) 
SELECT 
    PROFESSOR_NAME "교수이름",
...
SELECT
        ...
        STUDENT_NAME
FROM TB_STUDENT
WHERE TO_CHAR( ENTRANCE_DATE, 'YYYY') - TO_CHAR( TO_DATE(SUBSTR(STUDENT_SSN, 0, 2 ), 'YYYY' ), 'YYYY' ) > 19
ORDER BY 1;

-- 6. 2020 년 크리스마스는 무슨 요일인가? 
SELECT '2020년 크리스마스는 ' || TO_CHAR( TO_DATE('2020-12-25', 'YYYY-MM-DD'), 'DAY') 크리스마스요일 FROM DUAL;

-- 7. TO_DATE('99/10/11','YY/MM/DD'), TO_DATE('49/10/11','YY/MM/DD') 은 각각 몇 년 몇 
-- 월 몇 일을 의미할까? 또 TO_DATE('99/10/11', 'RR/MM/DD'), TO_DATE(' 49/10/11', 'RR/MM/DD') 은 각각 몇 년 몇월 몇 일을 의미할까? 
SELECT
...
SELECT 
    DEPARTMENT_NO 학과코드명, 
    SUM(DECODE(ABSENCE_YN,'Y','1','N','0')) 휴학생수 
FROM TB_STUDENT 
GROUP BY DEPARTMENT_NO 
ORDER BY 1; 
...
ORDER BY STUDENT_NAME ASC;
 ...
 SELECT
    SUBSTR(TERM_NO,3,2) "년도",
    SUBSTR(TERM_NO,5,2) "학기",
    ROUND(AVG(POINT),1) "평점"
FROM TB_GRADE
WHERE STUDENT_NO = 'A112113'
GROUP BY ROLLUP (SUBSTR( TERM_NO, 3, 2 ), substr( TERM_NO, 5, 2 ) )
order by 1, 2 asc;
