...
-- 3. 주소지가 강원도나 경기도인 학생들 중 1900 대 학번을 가진 학생들의 이름과 학번, 주소를 이름의 오름차순으로 화면에 출력하시오. 
...
-- (법학과의 '학과코드'는 학과 테이블(TB_DEPARTNENT)을 조회해서 찾아 내도록 하자) 
SELECT 
    A.PROFESSOR_NAME  이름, 
    A.PROFESSOR_SSN  주민번호 
FROM TB_PROFESSOR A 
JOIN TB_DEPARTMENT B ON A.DEPARTMENT_NO = B.DEPARTMENT_NO 
WHERE A.DEPARTMENT_NO LIKE '005'
ORDER BY 2 ASC;
...
SELECT 
    ...
    TO_CHAR(POINT,'9.99') 
FROM TB_GRADE A 
WHERE A.TERM_NO = 200402 
AND A.CLASS_NO LIKE 'C3118100' 
...
AND C.DEPARTMENT_NO = A.DEPARTMENT_NO
AND DEPARTMENT_NAME LIKE '음악학과'
GROUP BY A.STUDENT_NO, STUDENT_NAME
ORDER BY 3 DESC;
...
JOIN TB_PROFESSOR B ON B.DEPARTMENT_NO = A.DEPARTMENT_NO
JOIN TB_STUDENT C ON C.DEPARTMENT_NO = A.DEPARTMENT_NO
WHERE C.STUDENT_NO LIKE 'A313047';
...
SELECT 
    ...
    PROFESSOR_NO
FROM TB_CLASS A 
LEFT JOIN TB_DEPARTMENT B ON B.DEPARTMENT_NO=A.DEPARTMENT_NO 
LEFT JOIN TB_CLASS_PROFESSOR C ON C.CLASS_NO=A.CLASS_NO
WHERE CATEGORY LIKE '예체능' 
AND PROFESSOR_NO IS NULL;

-- 14. 춘 기술대학교 서반아어학과 학생들의 지도교수를 게시하고자 한다. 학생이름과 지도교수 이름을 찾고 만일 지도 교수가 없는 학생일 경우 "지도교수 미지정 " 으로 표시하도록 하는 SQL 문을 작성하시오. 
...
SELECT 
    A.STUDENT_NO 학번,
    A.STUDENT_NAME 이름,
    C.DEPARTMENT_NAME 학과,
    ROUND(AVG(POINT),1) 평점
FROM TB_STUDENT A 
LEFT JOIN TB_GRADE B ON B.STUDENT_NO = A.STUDENT_NO
...
FROM TB_STUDENT A 
JOIN TB_GRADE B ON A.STUDENT_NO = B.STUDENT_NO
WHERE A.DEPARTMENT_NO LIKE '001'
GROUP BY A.STUDENT_NO, A.STUDENT_NAME
HAVING AVG(POINT) = ( SELECT 
                        MAX(AVG(POINT)) 
                    FROM TB_GRADE C 
                    JOIN TB_STUDENT D ON D.STUDENT_NO= C.STUDENT_NO 
                    WHERE D.DEPARTMENT_NO LIKE '001' 
                    GROUP BY D.STUDENT_NO );

-- 19. 춘 기술대학교의 "환경조경학과"가 속한 같은 계열 학과들의 학과 별 전공과목 평점을 
--     파악하기 위한 적절한 SQL 문을 찾아내시오. 단, 출력헤더는 "계열 학과명", "전공평점"으로 표시되도록 하고, 평점은 소수점 한 자리까지만 반올림하여 표시되도록 한다. 
SELECT 
    A.DEPARTMENT_NAME 계열학과명,
    ROUND(AVG(POINT),1) 전공평점 
FROM TB_DEPARTMENT A 
JOIN TB_CLASS B ON A.DEPARTMENT_NO = B.DEPARTMENT_NO 
JOIN TB_GRADE C ON C.CLASS_NO = B.CLASS_NO 
WHERE CATEGORY = ( SELECT 
                        CATEGORY
                    FROM TB_DEPARTMENT 
                    WHERE DEPARTMENT_NAME LIKE '환경조경학과')
AND CLASS_TYPE IN ( SELECT 
                    CLASS_TYPE 
                    FROM TB_CLASS 
